...
async def test_obtener_inventario(async_client, api_key_header):
    with patch("routes.inventario_routes.obtener_producto", new_callable=AsyncMock) as mock_obtener_producto, \
         patch("routes.inventario_routes.inventario_collection.find_one") as mock_find_one:
...
async def test_actualizar_inventario(async_client, api_key_header):
    with patch("routes.inventario_routes.obtener_producto", new_callable=AsyncMock) as mock_obtener_producto, \
         patch("routes.inventario_routes.inventario_collection.update_one") as mock_update_one, \
         patch("routes.inventario_routes.inventario_collection.find_one") as mock_find_one:
...
async def test_obtener_inventario_timeout(async_client, api_key_header):
    with patch("routes.inventario_routes.obtener_producto", new_callable=AsyncMock, side_effect=httpx.RequestError("Timeout")) as mock_obtener:
        response = await async_client.get("/inventario/999", headers=api_key_header)
        assert response.status_code == 404
...
async def test_consultar_producto_no_encontrado(async_client, api_key_header):
    with patch("routes.inventario_routes.obtener_producto", return_value=None) as mock_obtener:
...
async def test_cliente_producto_http_status_error():
    with patch("services.productos_client.httpx.AsyncClient.get", side_effect=httpx.HTTPStatusError("Boom", request=None, response=None)) as mock_get:
        from services.productos_client import obtener_producto
